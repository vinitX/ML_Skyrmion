...
def monte(s,theta,phi,magnet):
    J=1
    D=6**0.5
    B=[0,0,3+magnet/10] #acting along z-axis
    T=0.15
    beta=1/T
    e=0.2#1
    update=0
...
for magnet in range(1,10):
    t = time.time()
    filename_a="theta"+str(6)+".txt" #building on existing data
    filename_b="phi"+str(6)+".txt"
    a=np.loadtxt(filename_a)
    theta=a.reshape(L,L)
    b=np.loadtxt(filename_b)
    phi=b.reshape(L,L)
    for m in range(50001):
        [s,theta,phi,step,update]=monte(s,theta,phi,magnet)
        if m%100==0 and m>20000:
            a=np.reshape(theta,(1,L*L))
            b=np.reshape(phi,(1,L*L))
            filename_a="theta"+str(3+magnet/10)+"_"+str(int(m/100)-200)+".txt"
            filename_b="phi"+str(3+magnet/10)+"_"+str(int(m/100)-200)+".txt"
            np.savetxt(filename_a,a)
            np.savetxt(filename_b,b)
        if m%100==0: print(int(m/100),end=' ')
    print("\n\n\n")
    print("Field : ",3+magnet/10)
    print("Acceptance Ratio: ",update/step)
    elapsed = time.time() - t
    print("Time elapsed for B = :",3+magnet/10," : ",elapsed)
    print("\n\n\n")