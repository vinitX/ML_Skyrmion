...
fval=zeros(length(K),length(D),length(H));
flag=zeros(length(K),length(D),length(H));
vec_p=zeros(length(K),length(D),length(H),size,2*size);

parfor x=1:length(K)
vecp_slice=zeros(length(D),length(H),size,2*size);
fval_slice=zeros(length(D),length(H));
flag_slice=zeros(length(D),length(H));
for y=1:length(D)
for z=1:length(H)
...
for i=1:size
    for j=1:size
        ...
        F=@(p)del*(F(p)+t1(p)^2/6-t1(p)+D(y)*t2(p)-(K(x)/2)*(1-s{3,i,j}(p)^2)+(H(z)/4)*(1-s{3,i,j}(p)));
    end
end
[vecp_slice(y,z,:,:),fval_slice(y,z),flag_slice(y,z)]=fminunc(F,p0); %Conjugate Gradiant Minimization
end
end
vec_p(x,:,:,:,:)=vecp_slice;
fval(x,:,:)=fval_slice;
...
save('square.mat','vec_p','fval','flag');
